...
def binomial_price(
    ...
    Performs the backward-induction recursion described in Sections 3 and
    4 of the accompanying note. For European options:

        V_n(j) = R^{-1} * [p * V_{n+1}(j+1) + (1-p) * V_{n+1}(j)]

    For American options, the recursion includes an early-exercise check:

        V_n(j) = max(payoff(S_n(j)), R^{-1} * [p * V_{n+1}(j+1) + (1-p) * V_{n+1}(j)])

    Args:
        S: Current spot price. Must be positive.
        K: Strike price. Must be positive.
        r: Continuously compounded riskless rate.
        params: TreeParameters dataclass specifying u, d, p, dt.
        option_type: Either "call" or "put".
        exercise_style: Either "european" or "american".
        q: Continuous dividend yield. Defaults to 0.

    Returns:
        The arbitrage-free time-0 price of the option.
    ...
    Examples:
        ...
        >>> params = crr_parameters(T=1, N=500, r=0.05, sigma=0.20)
        >>> price = binomial_price(
        ...     S=100, K=100, r=0.05, params=params,
        ...     option_type="call", exercise_style="european"
        ... )
        >>> # Should be very close to Black-Scholes value of 10.4506
        >>> abs(price - 10.4506) < 0.05
        True
    """
    if S <= 0:
    ...
    # Number of steps inferred from dt and the implicit horizon.
    # The caller specified T via params.dt; we recover N as round(T/dt).
    # In practice, we don't need N explicitly because we compute terminal
    # nodes directly from the tree structure.
    #
    # We need N to know the size of the terminal value array.
    # Convention: treat params.dt as fundamental and use N = round(T/dt) once,
    # but here T must be provided. We use a different convention: compute
    # N from the structure params.dt was derived from. Since the caller
    # constructed params with knowledge of T and N, we extract N by
    # requiring it as a separate argument? Better approach: extract it
    # from a convention that params.dt = T/N implies N from a known T.
    #
    # Cleanest design: require N explicitly. But params already contains dt.
    # So: the caller is responsible for ensuring T = N * params.dt is consistent.
    # We pass N inferred from a horizon; the simplest interface is to require T.
    raise NotImplementedError("Use the public function defined below.")
